...
import { useCheckoutModalContext } from "@/app/context/CheckoutModalContextType";
const CartSidebarModal = () => {
  const { isCartModalOpen, closeCartModal } = useCartModalContext();
  const cartItems = useAppSelector((state) => state.cartReducer.items);
  const { openCheckoutModal } = useCheckoutModalContext();
...
const handleOpenCheackoutModal = () => {
    openCheckoutModal();
  };
          ...
          <div className="border-t border-gray-3 bg-white pt-5 pb-4 sm:pb-7.5 lg:pb-11 mt-7.5 sticky bottom-0">
            ...
            <div className="flex items-center gap-4">
              <Link
                onClick={() => closeCartModal()}
                href="/cart"
                className="w-full flex justify-center font-medium text-white bg-orange py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-orange-dark"
              >
                View Cart
              </Link>

              <Link
                onClick={() => handleOpenCheackoutModal()}
                href={"/checkout"}
                className="w-full flex justify-center font-medium text-white bg-orange py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-orange-dark"
              >
                Checkout
