...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group bg-gray-2 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      {/* Product Image Container */}
      <div className="relative overflow-hidden flex items-center justify-center rounded-t-2xl bg-gradient-to-br from-gray-50 to-gray-100 min-h-[280px] mb-0">
        <Image 
        ...
        {item.price !== item.discountedPrice && (
          <div className="absolute top-3 left-3 bg-red text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-lg">
            -{Math.round(((item.price - item.discountedPrice) / item.price) * 100)}%
          </div>
        )}

        {/* Action Buttons - Enhanced with better positioning and effects */}
        <div className="absolute inset-x-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-3 pb-6 pt-4 bg-gradient-to-t from-black/20 to-transparent backdrop-blur-sm ease-out duration-300 group-hover:translate-y-0">
          {/* Quick View Button */}
          <button
            onClick={() => {
              openModal();
            ...
            className="flex items-center justify-center w-11 h-11 rounded-xl shadow-lg ease-out duration-200 text-gray-700 bg-white/90 backdrop-blur-sm hover:bg-white hover:text-orange
          ...
          <button
            ...
            className="inline-flex items-center font-semibold text-sm py-3 px-6 rounded-xl bg-gradient-to-r from-green to-orange text-white shadow-lg ease-out duration-200 hover:from-orange hover:to-orange hover:shadow-xl hover:scale-105 transform border border-orange"
          >
          ...
          <button
            ...
            className="flex items-center justify-center w-11 h-11 rounded-xl shadow-lg ease-out duration-200 text-gray-700 bg-white/90 backdrop-blur-sm hover:bg-white hover:text-red-500 hover:scale-110 border border-white/20"
          >
            <svg
              className="fill-current"
              width="18"
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-5 font-medium">({item.reviews})</span>
          </div>
        ...
        <Link href="/shop-details" onClick={() => handleProductDetails()}>
          <h3 className="font-semibold text-lg text-gray-7 leading-tight mb-3 line-clamp-2 hover:text-orange transition-colors duration-200 cursor-pointer">
            {item.title}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {item.price !== item.discountedPrice && (
              <span className="text-lg text-gray-7 line-through font-medium">
                ₹{item.price}
              </span>
            )}
          </div>
          ...
          {/* Savings Amount */}
          {item.price !== item.discountedPrice && (
            <span className="text-sm font-semibold text-green  px-2 py-1 rounded-md">
